...
class LoanAcceptanceTest {
    ...
    void 멀티_한도조회_3개_상품_결과_반환() throws Exception {
        mockMvc.perform(post("/v1/loans/inquiry")
                ...
                .andExpect(jsonPath("$.results[?(@.pdId == 1)].maxLoanAmt").value(50000000))
                .andExpect(jsonPath("$.results[?(@.pdId == 2)].maxLoanAmt").value(20000000))
                .andExpect(jsonPath("$.results[?(@.pdId == 3)].maxLoanAmt").value(15000000));
    }
}
