...
class ProductCommandServiceTest {
    ...
    void 등록_요청값_전달() {
        ProductCreateRequest request = request("신용대출", new BigDecimal("5.50"));
        Pd saved = fakePd(1L, "신용대출", new BigDecimal("5.50"));
        when(productRepository.save("신용대출", new BigDecimal("5.50"))).thenReturn(saved);

        sut.create(request);

        verify(productRepository).save("신용대출", new BigDecimal("5.50"));
    }
    ...
    void 등록_결과_매핑() {
        ProductCreateRequest request = request("신용대출", new BigDecimal("5.50"));
        Pd saved = fakePd(10L, "신용대출", new BigDecimal("5.50"));
        when(productRepository.save("신용대출", new BigDecimal("5.50"))).thenReturn(saved);
    ...
    private ProductCreateRequest request(String name, BigDecimal rate) {
